...
function PropertyValue({ definition, value }: { definition: PropertyDefinition; value: unknown }) {
  ...
  if (definition.type === "select") {
    return <span className="inline-flex h-[25px] items-center rounded-[13px] border border-[color-mix(in_srgb,var(--chip)_24%,transparent)] bg-[color-mix(in_srgb,var(--chip)_14%,var(--surface))] px-[9px] text-[11px] font-semibold text-[color-mix(in_srgb,var(--chip)_80%,var(--text))]" style={{ "--chip": option?.color || "#718096" } as React.CSSProperties}>{option ? localizedOptionLabel(option, locale) : String(value)}</span>;
  }
...
export function TaskCard({ task, selected, definitions, compact = false, onSelect, onQuickEdit }: Props) {
  ...
  return (
    <article className={cn("cursor-pointer rounded-xl border border-line bg-surface p-[15px] shadow-none transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--accent)_48%,var(--line))] hover:shadow-panel", selected && "border-accent shadow-[0_0_0_1px_var(--accent)]", compact && "px-3 py-2.5")} onClick={onSelect}>
      <div className={cn("mb-3 truncate text-[15px] font-[720]", compact && "mb-0 text-sm")} title={task.title}>{task.title}</div>
      {!compact ? (
